<?php
/**
 * Blossom Floral Typography Related Functions
 *
 * @package Blossom_Floral
 */

if( ! function_exists( 'blossom_floral_fonts_url' ) ):
/**
 * 
*/ 
function blossom_floral_fonts_url(){
    ...
    */
    $primary    = _x( 'on', 'Primary Font: on or off', 'blossom-floral' );
    $secondary  = _x( 'on', 'Secondary Font: on or off', 'blossom-floral' );
    $site_title = _x( 'on', 'Site Title Font: on or off', 'blossom-floral' );
...
if( ! function_exists( 'blossom_floral_get_google_fonts' ) ) :
/**
 * Get Google Fonts
*/
function blossom_floral_get_google_fonts(){
...
if( ! function_exists( 'blossom_floral_get_websafe_font' ) ) :
/**
 * Function listing WebSafe Fonts and its attributes
*/
function blossom_floral_get_websafe_font(){
    ...
    return apply_filters( 'blossom_floral_standard_fonts', $standard_fonts );
}
endif;

if( ! function_exists( 'blossom_floral_check_varient' ) ) :
/**
 * Checks for matched varients in google fonts for typography fields
*/
function blossom_floral_check_varient( $font_family = 'serif', $font_variants = 'regular', $body = false ){
    $variant = '';
    $var     = array();
    $google_fonts  = blossom_floral_get_google_fonts(); //Google Fonts
    $websafe_fonts = blossom_floral_get_websafe_font(); //Standard Web Safe Fonts
...
*/
function blossom_floral_get_css_variant( $font_variant ){
    $v_array = array(
		'100'       => array(
            'weight'    => '100',
            'style'     => 'normal'
...
*/
function blossom_floral_is_google_font( $font ){
    $return = false;
    $websafe_fonts = blossom_floral_get_websafe_font();
    if( $font ){
...
*/
function blossom_floral_get_fonts( $font_family, $font_variant ){
    
    $fonts = array();
    $websafe_fonts = blossom_floral_get_websafe_font(); //Web Safe Font
    
    if( $font_family ){
        if( blossom_floral_is_google_font( $font_family ) ){
            $fonts['font'] = esc_attr( $font_family ); //Google Font
            if( $font_variant ){
                $weight_style    = blossom_floral_get_css_variant( blossom_floral_check_varient( $font_family, $font_variant ) );
...
if( ! function_exists( 'blossom_floral_get_all_fonts' ) ) :
/**
 * Return Web safe font and google font without variants
*/
function blossom_floral_get_all_fonts(){
    $google = array();        
    $standard = apply_filters( 'blossom_floral_standard_font', array(
		'georgia-serif'       => __( 'Georgia', 'blossom-floral' ),
        'palatino-serif'      => __( 'Palatino Linotype, Book Antiqua, Palatino', 'blossom-floral' ),
        'times-serif'         => __( 'Times New Roman, Times', 'blossom-floral' ),
        'arial-helvetica'     => __( 'Arial, Helvetica', 'blossom-floral' ),
        'arial-gadget'        => __( 'Arial Black, Gadget', 'blossom-floral' ),
		'comic-cursive'       => __( 'Comic Sans MS, cursive', 'blossom-floral' ),
		'impact-charcoal'     => __( 'Impact, Charcoal', 'blossom-floral' ),
        'lucida'              => __( 'Lucida Sans Unicode, Lucida Grande', 'blossom-floral' ),
        'tahoma-geneva'       => __( 'Tahoma, Geneva', 'blossom-floral' ),
		'trebuchet-helvetica' => __( 'Trebuchet MS, Helvetica', 'blossom-floral' ),
		'verdana-geneva'      => __( 'Verdana, Geneva', 'blossom-floral' ),
        'courier'             => __( 'Courier New, Courier', 'blossom-floral' ),
        'lucida-monaco'       => __( 'Lucida Console, Monaco', 'blossom-floral' ),
	) );
